...
const CartItemCard = ({ item, deleteCartItems }) => {
  ...
  return (
    <div className=" block md:flex md:justify-around md:items-start box-border my-5">
      <img src={P3} alt="item" className=" w-[200px] md:w-[180px]"/>
      {/* <img src={item.image} alt="item" /> */}
      <div className=" flex flex-col justify-center m-auto">
        <div onClick={() => {history.push(`/product/${item.product}`)}} className=" cursor-pointer">{item.name}</div>
        ...
        onClick={() => deleteCartItems(item.product)}>Remove</Button>
      </div>
    </div>
  )
}
